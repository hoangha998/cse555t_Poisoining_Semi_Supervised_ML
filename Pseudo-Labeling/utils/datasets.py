#!coding:utf-8
import numpy as np
import torchvision as tv
import torchvision.transforms as transforms

from utils.randAug import RandAugmentMC
from utils.data_utils import NO_LABEL
from utils.data_utils import TransformWeakStrong as wstwice

import sys
BASE_DIR = '/content/drive/MyDrive/AdversarialAI/'
sys.path.insert(1, BASE_DIR + 'Poisoner')
from Poisoner import Poisoner
from get_datasets import get_subset_cifar10, get_poison_dataset_from_images
from torch.utils.data import ConcatDataset, DataLoader



load = {}
def register_dataset(dataset):
    def warpper(f):
        load[dataset] = f
        return f
    return warpper

def encode_label(label):
    return NO_LABEL* (label +1)

def decode_label(label):
    return NO_LABEL * label -1

def split_relabel_data(np_labs, labels, label_per_class,
                        num_classes):
    """ Return the labeled indexes and unlabeled_indexes
    """
    labeled_idxs = []
    unlabed_idxs = []
    for id in range(num_classes):
        indexes = np.where(np_labs==id)[0]
        np.random.shuffle(indexes)
        labeled_idxs.extend(indexes[:label_per_class])
        unlabed_idxs.extend(indexes[label_per_class:])
    np.random.shuffle(labeled_idxs)
    np.random.shuffle(unlabed_idxs)
    ## relabel dataset
    # for idx in unlabed_idxs:
    #     labels[idx] = encode_label(labels[idx])

    return labeled_idxs, unlabed_idxs


# @register_dataset('cifar10')
# def cifar10(n_labels, data_root='./data-local/cifar10/'):
#     channel_stats = dict(mean = [0.4914, 0.4822, 0.4465],
#                 std = [0.2023, 0.1994, 0.2010])
#     train_transform = transforms.Compose([
#         transforms.Pad(2, padding_mode='reflect'),
#         transforms.ColorJitter(brightness=0.4, contrast=0.4,
#                                saturation=0.4, hue=0.1),
#         transforms.RandomCrop(32),
#         transforms.RandomHorizontalFlip(),
#         transforms.ToTensor(),
#         transforms.Normalize(**channel_stats)
#     ])
#     eval_transform = transforms.Compose([
#         transforms.ToTensor(),
#         transforms.Normalize(**channel_stats)
#     ])
    
#     trainset = tv.datasets.CIFAR10(data_root, train=True, download=True,
#                                    transform=train_transform)
#     evalset = tv.datasets.CIFAR10(data_root, train=False, download=True,
#                                    transform=eval_transform)
#     num_classes = 10
#     label_per_class = n_labels // num_classes
#     labeled_idxs, unlabed_idxs = split_relabel_data(
#                                     np.array(trainset.targets),
#                                     trainset.targets,
#                                     label_per_class,
#                                     num_classes)
#     print(f'label_idxs: {len(labeled_idxs)}')
#     print(f'unlab_idxs: {len(unlabed_idxs)}')
    
#     return {
#         'trainset': trainset,
#         'evalset': evalset,
#         'label_idxs': labeled_idxs,
#         'unlab_idxs': unlabed_idxs,
#         'num_classes': num_classes
#     }



class CustomConcatDataset(ConcatDataset):
    def __init__(self, datasets):
        super().__init__(datasets)
        self.targets = []
        for dataset in datasets:
            if hasattr(dataset, 'targets'):
                self.targets.extend(dataset.targets)

@register_dataset('cifar10')
def cifar10(n_labels, data_root='./data-local/cifar10/'):
    channel_stats = dict(mean = [0.4914, 0.4822, 0.4465],
                std = [0.2023, 0.1994, 0.2010])
    train_transform = transforms.Compose([
        transforms.Pad(2, padding_mode='reflect'),
        transforms.ColorJitter(brightness=0.4, contrast=0.4,
                    saturation=0.4, hue=0.1),
        transforms.RandomCrop(32),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize(**channel_stats)
    ])
    eval_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(**channel_stats)
    ])
    
    train_labeled, train_unlabeled, test_dataset = get_subset_cifar10(subset_size=30000,
                                    labeled_size=400, 
                                    test_size=3000,
                                    seed=0)

    poisoner = Poisoner()
    # num_poison_samples = int(0.1 * len(train_unlabeled))  # Calculate the number of poison samples and ensure it's an integer
    # poisoned_dataset = poisoner.generate_poison_dataset(train_labeled, train_unlabeled, N=num_poison_samples)
  
    poisoned_dataset = poisoner.generate_poison_dataset(train_labeled, train_unlabeled, label_pair=[2,6], N=400)


    poisoned_dataset.transform = train_transform
    poisoned_dataset.asPIL = True
    train_labeled.transform = train_transform
    train_labeled.asPIL = True
    train_unlabeled.transform = train_transform
    train_unlabeled.asPIL = True

    trainset = CustomConcatDataset([train_labeled, train_unlabeled])
    targets = np.concatenate([train_labeled.targets,train_unlabeled.targets])
    
    num_classes = 10
    label_per_class = n_labels // num_classes
    labeled_idxs, unlabed_idxs = split_relabel_data(
                    targets,
                    targets,
                    label_per_class,
                    num_classes) 
    poisoned_idx = [_ for _ in range(len(trainset), len(trainset)+len(poisoned_dataset))]
    # print(f"unlab_idxs:{unlabed_idxs}")
    unlabed_idxs = unlabed_idxs + poisoned_idx

    for idx in range(len(train_unlabeled)):
      train_unlabeled.targets[idx] = encode_label(train_unlabeled.targets[idx])
    
    trainset = CustomConcatDataset([train_labeled, train_unlabeled, poisoned_dataset])
    for i in unlabed_idxs:
      trainset.targets[i] = encode_label(trainset.targets[i])

    evalset = test_dataset
    evalset.asPIL = True
    evalset.transform = eval_transform
    # print(f'label_idxs: {labeled_idxs}')
    # print(f'unlab_idxs: {unlabed_idxs}')
    # print("sizes:", len(trainset), len(train_labeled), len(train_unlabeled), len(poisoned_dataset))

  
    return {
        'trainset': trainset,
        'evalset': evalset,
        'label_idxs': labeled_idxs,
        'unlab_idxs': unlabed_idxs,
        'num_classes': num_classes
    }


# class CustomConcatDataset(ConcatDataset):
#     def __init__(self, datasets):
#         super().__init__(datasets)
#         self.targets = []
#         for dataset in datasets:
#             if hasattr(dataset, 'targets'):
#                 self.targets.extend(dataset.targets)

# @register_dataset('cifar10')
# def cifar10(n_labels, data_root='./data-local/cifar10/'):
#     channel_stats = dict(mean = [0.4914, 0.4822, 0.4465],
#                 std = [0.2023, 0.1994, 0.2010])
#     train_transform = transforms.Compose([
#         transforms.Pad(2, padding_mode='reflect'),
#         transforms.ColorJitter(brightness=0.4, contrast=0.4,
#                                saturation=0.4, hue=0.1),
#         transforms.RandomCrop(32),
#         transforms.RandomHorizontalFlip(),
#         transforms.ToTensor(),
#         # transforms.Lambda(lambda x: x.float()),
#         transforms.Normalize(**channel_stats)
#     ])
#     eval_transform = transforms.Compose([
#         transforms.ToTensor(),
#         # transforms.Lambda(lambda x: x.float()),
#         transforms.Normalize(**channel_stats)
#     ])
    
#     train_labeled, train_unlabeled, test_dataset = get_subset_cifar10(subset_size=20000,
#                                     labeled_size=15000, 
#                                     test_size=5000,
#                                     seed=0)

#     poisoner = Poisoner()
#     num_poison_samples = int(0.1 * len(train_unlabeled))  # Calculate the number of poison samples and ensure it's an integer
#     poisoned_dataset = poisoner.generate_poison_dataset(train_labeled, train_unlabeled, N=num_poison_samples)

#     poisoned_dataset.transform = train_transform
#     poisoned_dataset.asPIL = True
#     train_labeled.transform = train_transform
#     train_labeled.asPIL = True
#     train_unlabeled.transform = train_transform
#     train_unlabeled.asPIL = True


#     trainset = CustomConcatDataset([train_labeled, train_unlabeled])
    
#     num_classes = 10
#     label_per_class = n_labels // num_classes
#     labeled_idxs, unlabed_idxs = split_relabel_data(
#                     np.array(trainset.targets),
#                     trainset.targets,
#                     label_per_class,
#                     num_classes) 

#     trainset = CustomConcatDataset([train_labeled, train_unlabeled, poisoned_dataset])
#     evalset = test_dataset
#     evalset.asPIL = True
#     evalset.transform = eval_transform


#     return {
#         'trainset': trainset,
#         'evalset': evalset,
#         'label_idxs': labeled_idxs,
#         'unlab_idxs': unlabed_idxs,
#         'num_classes': num_classes
#     }




@register_dataset('wscifar10')
def wscifar10(n_labels, data_root='./data-local/cifar10/'):
    channel_stats = dict(mean = [0.4914, 0.4822, 0.4465],
                         std = [0.2023, 0.1994, 0.2010])
    weak = transforms.Compose([
        transforms.RandomHorizontalFlip(),
        transforms.Pad(2, padding_mode='reflect'),
        transforms.RandomCrop(32),
        transforms.ToTensor(),
        transforms.Normalize(**channel_stats)
    ])
    strong = transforms.Compose([
        transforms.RandomHorizontalFlip(),
        transforms.Pad(2, padding_mode='reflect'),
        transforms.RandomCrop(32),
        RandAugmentMC(n=2, m=10),
        transforms.ToTensor(),
        transforms.Normalize(**channel_stats)
    ])
    train_transform = wstwice(weak, strong)
    eval_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(**channel_stats)
    ])
    trainset = tv.datasets.CIFAR10(data_root, train=True, download=True,
                                   transform=train_transform)
    evalset = tv.datasets.CIFAR10(data_root, train=False, download=True,
                                   transform=eval_transform)
    num_classes = 10
    label_per_class = n_labels // num_classes
    labeled_idxs, unlabed_idxs = split_relabel_data(
                                    np.array(trainset.train_labels),
                                    trainset.train_labels,
                                    label_per_class,
                                    num_classes)
    return {
        'trainset': trainset,
        'evalset': evalset,
        'label_idxs': labeled_idxs,
        'unlab_idxs': unlabed_idxs,
        'num_classes': num_classes
    }


@register_dataset('cifar100')
def cifar100(n_labels, data_root='./data-local/cifar100/'):
    channel_stats = dict(mean = [0.5071, 0.4867, 0.4408],
                         std = [0.2675, 0.2565, 0.2761])
    train_transform = transforms.Compose([
        transforms.Pad(2, padding_mode='reflect'),
        transforms.ColorJitter(brightness=0.4, contrast=0.4,
                               saturation=0.4, hue=0.1),
        transforms.RandomCrop(32),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize(**channel_stats)
    ])
    eval_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(**channel_stats)
    ])
    trainset = tv.datasets.CIFAR100(data_root, train=True, download=True,
                                   transform=train_transform)
    evalset = tv.datasets.CIFAR100(data_root, train=False, download=True,
                                   transform=eval_transform)
    num_classes = 100
    label_per_class = n_labels // num_classes
    labeled_idxs, unlabed_idxs = split_relabel_data(
                                    np.array(trainset.train_labels),
                                    trainset.train_labels,
                                    label_per_class,
                                    num_classes)
    return {
        'trainset': trainset,
        'evalset': evalset,
        'labeled_idxs': labeled_idxs,
        'unlabeled_idxs': unlabed_idxs,
        'num_classes': num_classes
    }
