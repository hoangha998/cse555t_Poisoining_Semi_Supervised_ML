...
def main():
    ...
    parser.add_argument('--test-size', type=int, default=2000,
                        help='number of labeled data')
    parser.add_argument('--labeled-size', type=int, default=0.05,
                        help='number of labeled data')
    parser.add_argument('--poison-output', type=str, default='latest_poison',
                        help="poison folder name to add to ./data/poison/")
    parser.add_argument('--poison-size', type=int, default=50,
                        help='number of poisoned samples to inject')
    parser.add_argument('--label-mali', type=int, default=-1,
    ...
    if args.resume == '': # from scratch
        ...
        poison_dataset = poisoner.generate_poison_dataset(train_labeled, train_unlabeled, N=args.poison_size, 
                                                        output_folder=args.poison_output, label_pair = label_pair)
    elif args.poison_size > 0: # retreive poison dataset
        injected_folder_path = os.path.join(args.poison_output, "injected")
        poison_dataset = get_poison_dataset_from_images(injected_folder_path)
        print("retrieved injected poison at", injected_folder_path)
    ...
    labeled_trainloader = DataLoader(
        train_labeled,
        sampler=train_sampler(train_labeled),
    ...
    if args.poison_size > 0:
        ...
        combined_unlabeled = ConcatDataset([train_unlabeled, poison_dataset])
        print("added poison to the unlabeled dataset")
    else:
        combined_unlabeled = train_unlabeled

    unlabeled_trainloader = DataLoader(
        combined_unlabeled, 
        sampler=train_sampler(combined_unlabeled),
